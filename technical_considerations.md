...
- latest visual-regression follow-up: nightly now includes a masked framebuffer ROI pixel-baseline gate (`qemu-smoke-gui-visual-baseline`, expected `fb-shell-v4` hash `0x1BD7880D`) plus a paired refresh helper (`qemu-smoke-gui-visual-baseline-refresh`)
- latest GUI telemetry follow-up: framebuffer footer now renders compact command latency telemetry (`last/avg/peak`, outcome, and in-flight run ticks) from existing shell lifecycle parsing; shell-core smoke now asserts emitted `display: cmd_latency ...` lines
- latest GUI telemetry follow-up: header metrics now include compact command health (`ok/fail`, fail streak, recent command rate), prompt input state emits a subtle failure-recovery cue (`CHK <n>`), and shell-core smoke now asserts emitted `display: cmd_health ...` lines
- latest GUI telemetry follow-up: command outcomes now feed an `OK/WARN/DEGR` state machine (fail streak + success-rate thresholds), with footer/prompt state cues and deterministic `display: cmd_health_state ...` transition logs asserted by shell-core smoke
- latest GUI telemetry follow-up: command outcomes now also maintain a rolling success-quality window (`recent %`), surfaced in header/footer HUD (`R%`) and emitted as deterministic `display: cmd_health_window ...` logs asserted by shell-core smoke
- latest GUI telemetry follow-up: health-state transitions now track recovery timing from `WARN/DEGR` back to `OK` (last/avg/peak `X` ticks), emit deterministic `display: cmd_health_recovery ...` logs, and expose compact recovery telemetry in HUD metrics
- latest GUI telemetry follow-up: command completion now also emits adaptive latency-budget classification (`display: cmd_latency_budget ...`), with header/footer HUD surfacing slow-command counters/status (`L`, `B`, `S`) without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell history now persists to `/persist/.sh_history` when writable storage is available, includes a builtin management command (`history clear`), retries persistence after transient write-open failures, and is replay/clear-validated across reboots by nightly `qemu-smoke-shell-history-persist`
- latest Phase 12 follow-up: shell input now also supports reverse history search (`Ctrl+R`) with repeat-cycle behavior and deterministic `search: ...` hint output, asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell prompt editing now supports `Ctrl+U` (clear line) and `Ctrl+W` (erase previous word) as deterministic input accelerators, asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell now supports history event recall (`!!` and `!N` within bounded history window), emits deterministic `history: <event> -> <command>` hints, and is asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell history event recall now also supports relative/pattern selectors (`!-N`, `!prefix`, `!?term`) with deterministic expansion hints, asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell now supports quick history substitution (`^old^new^`) against the latest command with deterministic expansion hints, asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: history browse is now prefix-filtered when editing (`Esc`/`Ctrl+P` and `Ctrl+N` traverse matching entries only and restore typed prefix when exiting), asserted by `qemu-smoke-shell-core` without syscall/parser/ABI growth
- latest Phase 12 follow-up: shell now supports indexed replay via `history run N` (matching displayed history numbering), with deterministic `history: run N -> ...` hints and replay-output assertions in `qemu-smoke-shell-core`, still without syscall/parser/ABI growth
