...
- `KERNEL_FRAMEBUFFER_WINDOW_BASE = 0xC2000000`
- `KERNEL_FRAMEBUFFER_WINDOW_SIZE_BYTES = 16 MiB`
...
- A reserved higher-half framebuffer/MMIO window now starts at `0xC2000000` and spans `16 MiB`
...
- Dynamic page tables allocated for the framebuffer window currently come from the heap, so that heap must remain inside the early mirrored region.
...
- Extend the new kernel VMM mapping path beyond the framebuffer/device window and teach it to reclaim or reuse dynamic page-table state
